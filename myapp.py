...
st.set_page_config(
    page_title='Analytics Service Portal',
    page_icon='📈'
)
#title
st.title(':rainbow[Data Analytics Portal]')
st.subheader(':grey[Explore data at its best.]', divider='rainbow')
#fileuploading
file = st.file_uploader('Drop your Excel or CSV file', type=['csv','xlsx'])
if(file!=None) :
    ...
    st.subheader(':rainbow[Basic Information of the File]',divider='rainbow')
    tab1,tab2,tab3,tab4 = st.tabs(['Summary','Top and Bottom Rows','Data Types','Columns'])

    with tab1:
        st.write(f'there are {data.shape[0]} rows in dataset and {data.shape[1]} columns in the dataset')
        st.subheader(':grey[Statistical Summary of dataset]')
    ...
    st.write('Groupby is a data analysis function that groups data based on specific categories and applies aggregate operations like sum, count, or average.'
            'It helps in summarizing large datasets efficiently by organizing similar data points together for easier analysis.')
    with st.expander('Group By your columns'):
        ...
        with col3:
            operation = st.selectbox('Choose operation',options=['sum','max','mean','median','min','count'])
